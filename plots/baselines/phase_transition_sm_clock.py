"""Combined boundary-aligned SM clock transition plots for baseline runs."""
...
OUTPATH = Path("plots/out/baselines/phase_transition_sm_clock.png")
BIN_WIDTH_S = 0.5
WINDOW_LEFT_S = -20.0
WINDOW_RIGHT_S = 40.0
...
def _build_transition_samples(
    ...
    if cross_step:
        # training -> rollout crosses canonical steps, so align by the rollout boundary
        # timestamp itself rather than requiring periodic samples to share the rollout step.
...
def _aggregate_bins(df: pd.DataFrame, transition_key: str) -> pd.DataFrame:
    ...
    for (policy_norm, bin_center_s), g in df.groupby(["policy_norm", "bin_center_s"], dropna=False):
        clock = pd.to_numeric(g["sm_clock_MHz"], errors="coerce").dropna().to_numpy(dtype=float)
        if clock.size == 0:
            continue
        ...
        rows.append(
            {
                ...
                "clock_median": float(np.median(clock)),
                "clock_q25": float(np.percentile(clock, 25)),
                "clock_q75": float(np.percentile(clock, 75)),
...
def main() -> None:
    ...
    y_max = float(all_binned["clock_q75"].max()) if len(all_binned) else 1.0
    y_min = float(all_binned["clock_q25"].min()) if len(all_binned) else 0.0
    y_pad = max(30.0, 0.05 * (y_max - y_min))
    band_height = 0.025
    band_gap = 0.008
    ...
    for row_idx, transition in enumerate(TRANSITIONS):
        ...
        if not sub.empty:
            for policy_idx, policy_norm in enumerate(TARGET_POLICIES):
                ...
                med = pol["clock_median"].to_numpy(dtype=float)
                ax.plot(
                    x,
                    med,
                    color=color,
    ...
    fig.suptitle("SM Clock Change Across Phase Transitions, Llama-3.1-8B-Inst", y=0.99, fontweight="bold")
