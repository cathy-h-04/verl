"""Baseline PCIe throughput by phase, aggregated across baseline policies."""
...
OUTPATH = Path("plots/out/baselines/non_results/phase_pcie_throughput.png")
...
def main() -> None:
    ...
    for facet in TARGET_MODEL_FACETS:
        ...
        tx_vals = [
            float(sub.loc[sub["phase_name"] == p, "tx_mean_bytes_s"].iloc[0]) / THROUGHPUT_SCALE
            if (sub["phase_name"] == p).any()
            else np.nan
            for p in PHASE_ORDER
        ]
        rx_vals = [
            float(sub.loc[sub["phase_name"] == p, "rx_mean_bytes_s"].iloc[0]) / THROUGHPUT_SCALE
            if (sub["phase_name"] == p).any()
            else np.nan
            for p in PHASE_ORDER
        ]
        ax.bar(x - wbar / 2, tx_vals, width=wbar, color=TX_COLOR, edgecolor="black", linewidth=0.7)
        ax.bar(x + wbar / 2, rx_vals, width=wbar, color=RX_COLOR, edgecolor="black", linewidth=0.7)
        ax.set_xticks(x)
        ax.set_xticklabels([PHASE_DISPLAY[p] for p in PHASE_ORDER])
        ax.set_xlabel("Phase")
        ax.set_title(MODEL_DISPLAY[facet], fontweight="bold")
        ax.grid(axis="y", alpha=0.2)

    axes[0].set_ylabel("Mean PCIe throughput (GB/s)")
    fig.suptitle(
        "Baseline Mean PCIe Throughput by Phase and Model",
        y=0.99,
        fontweight="bold",
    )
    legend_handles = [
    ...
    fig.legend(handles=legend_handles, frameon=False, loc="upper center", ncol=2, bbox_to_anchor=(0.5, 0.95))
