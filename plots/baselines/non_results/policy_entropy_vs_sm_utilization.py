"""Policy entropy versus SM utilization during training."""
...
OUTPATH = Path("plots/out/baselines/non_results/policy_entropy_vs_sm_utilization.png")
...
def main() -> None:
    ...
    entropy_df = sml[sml["metric_key"] == ENTROPY_KEY][["run_id", "global_step_canonical", "metric_value_float"]].copy()
    entropy_df = entropy_df.rename(columns={"metric_value_float": "actor_entropy"})
    entropy_df = entropy_df.merge(eligible, on=["run_id", "global_step_canonical"], how="inner")

    hardware_periodic, _ = load_view("hardware_periodic")
    hp = hardware_periodic[
        hardware_periodic["run_id"].astype(str).isin(selected_run_ids)
        & (hardware_periodic["record_type"].astype(str).str.upper() == "PERIODIC")
        & (hardware_periodic["source"].astype(str).str.lower() == "nvml")
    ].copy()
    hp["global_step_canonical"] = pd.to_numeric(hp["global_step_canonical"], errors="coerce")
    hp["sm_util_pct"] = pd.to_numeric(hp["sm_util_pct"], errors="coerce")
    hp["phase_name"] = hp["phase_name"].astype(str).str.lower()
    hp = hp.dropna(subset=["global_step_canonical", "sm_util_pct"]).copy()
    hp["global_step_canonical"] = hp["global_step_canonical"].astype(int)
    hp = hp[(hp["phase_name"] == "training")].merge(eligible, on=["run_id", "global_step_canonical"], how="inner")
    sm_df = (
        hp.groupby(["run_id", "global_step_canonical"], as_index=False)["sm_util_pct"]
        .mean()
        .rename(columns={"sm_util_pct": "training_sm_util_pct"})
    )

    plot_df = entropy_df.merge(
        sm_df,
    ...
    if plot_df.empty:
        raise ValueError("No rows after joining actor entropy and training SM utilization.")
    ...
    axes[0].set_ylabel("Mean training-phase sm_util_pct")
    handles, labels = axes[0].get_legend_handles_labels()
    fig.suptitle("Policy Entropy vs SM Utilization During Training", y=0.99, fontweight="bold")
