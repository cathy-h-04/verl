...
OUTPATH = Path("plots/out/reward_model/sm_clock_over_time.png")
GRID_DT_S = 0.1
...
ROLLOUT_TO_RL_POLICY_COLOR = "#000000"
ITERATION_BOUNDARY_COLOR = "#D62728"
...
def main() -> None:
    ...
    df = df.dropna(subset=["ts_monotonic_ns", "sm_clock_MHz"]).copy()
    df["ts_monotonic_ns"] = df["ts_monotonic_ns"].astype(np.int64)
    window_starts = (
        df.groupby(["run_id", "global_step_canonical"], as_index=False)["ts_monotonic_ns"]
        .min()
        .rename(columns={"ts_monotonic_ns": "window_start_ts"})
    )
    ...
    phase_fact["global_step_canonical"] = pd.to_numeric(phase_fact["global_step_canonical"], errors="coerce")
    rollout_to_rl_policy = phase_fact[
        (phase_fact["phase_name"] == "rl_policy") & (phase_fact["global_step_canonical"].isin(TARGET_ITERATIONS))
    ][["run_id", "global_step_canonical", "phase_start_ts"]].copy()
    rollout_to_rl_policy["phase_start_ts"] = pd.to_numeric(rollout_to_rl_policy["phase_start_ts"], errors="coerce")
    rollout_to_rl_policy = rollout_to_rl_policy.dropna(subset=["phase_start_ts"]).copy()
    rollout_to_rl_policy["phase_start_ts"] = rollout_to_rl_policy["phase_start_ts"].astype(np.int64)
    rollout_to_rl_policy = rollout_to_rl_policy.merge(
        window_starts,
        on=["run_id", "global_step_canonical"],
        how="inner",
        validate="one_to_one",
    ).merge(
        selected_runs[["run_id", "policy_norm"]],
        on="run_id",
        how="inner",
        validate="many_to_one",
    )
    rollout_to_rl_policy["boundary_s"] = (
        rollout_to_rl_policy["phase_start_ts"] - rollout_to_rl_policy["window_start_ts"]
    ) / 1e9

    window_frames = []
    for (_run_id, _step), g in df.groupby(["run_id", "global_step_canonical"], dropna=False):
        win = _resample_run_iteration_window(g, metric_col="sm_clock_MHz", dt_s=GRID_DT_S)
        if not win.empty:
    ...
    rollout_boundary_positions = (
        rollout_to_rl_policy.groupby(["policy_norm", "global_step_canonical"], as_index=False)["boundary_s"]
        .median()
        .rename(columns={"global_step_canonical": "iteration"})
    )
    rollout_boundary_positions["x_s"] = rollout_boundary_positions.apply(
        lambda row: float(row["boundary_s"]) + offsets[int(row["iteration"])],
        axis=1,
    )
    ...
    for ax, policy in zip(axes, TARGET_POLICIES):
        ...
        policy_rollout_boundaries = rollout_boundary_positions[
            rollout_boundary_positions["policy_norm"] == policy
        ].sort_values("iteration")
        for boundary_x in policy_rollout_boundaries["x_s"].to_numpy(dtype=float):
            ax.axvline(boundary_x, color=ROLLOUT_TO_RL_POLICY_COLOR, linewidth=1.6, alpha=0.95)
        if len(boundaries) > 1:
            ax.axvline(boundaries[1][1], color=ITERATION_BOUNDARY_COLOR, linewidth=1.3, alpha=0.95)
    ...
    handles.extend(
        [
            Line2D(
                [0],
                [0],
                color=ROLLOUT_TO_RL_POLICY_COLOR,
                linestyle="-",
                lw=1.6,
                label="rollout -> rl_policy boundary",
            ),
            Line2D(
                [0],
                [0],
                color=ITERATION_BOUNDARY_COLOR,
                linestyle="-",
                lw=1.3,
                label="iteration 29 start",
            ),
        ]
    )
    fig.legend(handles=handles, frameon=False, ncol=2, loc="upper center", bbox_to_anchor=(0.5, 0.94))
    fig.suptitle("SM Clock Over Time by Policy and Reward Mechanism (Iterations 28-29)", fontweight="bold", y=0.985)
