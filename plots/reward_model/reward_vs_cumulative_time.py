"""Validation reward versus cumulative runtime for reward-mechanism runs."""
...
OUTPATH = Path("plots/out/reward_model/reward_vs_cumulative_time.png")
...
def main() -> None:
    ...
    required_step = ["run_id", "global_step_canonical", "step_time_s"]
    ...
    if plot_df.empty:
        raise ValueError("No reward trajectory rows after joining validation reward to cumulative runtime.")

    plot_df = plot_df.sort_values(["experiment_facet", "policy_norm", "global_step_canonical"])

    final_points = (
        plot_df.sort_values(["run_id", "global_step_canonical"])
        .groupby(["experiment_facet", "policy_norm"], as_index=False)
        .tail(1)
    )
    final_pivot = final_points.pivot_table(
        index="policy_norm",
        columns="experiment_facet",
        values="metric_value_float",
        aggfunc="first",
    )
    percent_delta_lines: list[str] = []
    for policy in TARGET_POLICIES:
        if policy not in final_pivot.index:
            continue
        if not {"Llama Reward Function", "Llama Reward Model"}.issubset(set(final_pivot.columns)):
            continue
        rf = pd.to_numeric(final_pivot.loc[policy, "Llama Reward Function"], errors="coerce")
        rm = pd.to_numeric(final_pivot.loc[policy, "Llama Reward Model"], errors="coerce")
        if pd.isna(rf) or pd.isna(rm) or rf == 0:
            continue
        delta_pct = (float(rm) - float(rf)) / float(rf) * 100.0
        percent_delta_lines.append(f"{POLICY_DISPLAY[policy]} {delta_pct:+.1f}%")

    print("rows used for plotting:")
    print(
        plot_df[
            ["run_id", "experiment_facet", "policy_norm", "global_step_canonical", "cumulative_time_h", "metric_value_float"]
    ...
    for ax, facet in zip(axes, TARGET_EXPERIMENT_FACETS):
        ...
        for policy in TARGET_POLICIES:
            ...
            ax.scatter(
                [float(best_row["cumulative_time_h"])],
    ...
    if percent_delta_lines:
        axes[1].text(
            0.03,
            0.97,
            "RM vs RF final reward Δ%\n" + "\n".join(percent_delta_lines),
            transform=axes[1].transAxes,
            ha="left",
            va="top",
            fontsize=8,
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "white", "alpha": 0.78, "edgecolor": "#666666"},
        )
    ...
    fig.suptitle("Validation Reward vs Cumulative Runtime by Reward Mechanism", y=0.99, fontweight="bold")
