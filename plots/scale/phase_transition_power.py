"""Combined boundary-aligned GPU power transition plots for scaling runs."""
...
OUTPATH = Path("plots/out/scale/phase_transition_power.png")
BIN_WIDTH_S = 0.5
WINDOW_LEFT_S = -20.0
WINDOW_RIGHT_S = 40.0

TARGET_ARCHES = ("a100", "h200")
ARCH_TITLE = {
    "a100": "A100",
    "h200": "H200",
}
...
def _select_scaling_runs() -> pd.DataFrame:
    ...
    runs_df["arch"] = runs_df["run_id"].astype(str).str.extract(r"_(a100|h200)_", expand=False).str.lower()
    selected = runs_df[is_scaling & checkpoint_mask & runs_df["arch"].isin(TARGET_ARCHES)][["run_id", "arch"]].drop_duplicates()
    if selected.empty:
...
def _build_transition_samples(
    ...
    df["gpu_power_mW"] = pd.to_numeric(df["gpu_power_mW"], errors="coerce") * 1e-3
    df["thr_sw_power_cap"] = df["thr_sw_power_cap"].astype("boolean").fillna(False).astype(bool)
    df = df.dropna(subset=["ts_monotonic_ns", "gpu_power_mW"]).copy()
...
def _aggregate_bins(df: pd.DataFrame, transition_key: str) -> pd.DataFrame:
    rows = []
    for (arch, bin_center_s), g in df.groupby(["arch", "bin_center_s"], dropna=False):
        vals = pd.to_numeric(g["gpu_power_mW"], errors="coerce").dropna().to_numpy(dtype=float)
        if vals.size == 0:
            continue
        ...
        rows.append(
            {
                ...
                "power_median": float(np.median(vals)),
                "power_q25": float(np.percentile(vals, 25)),
                "power_q75": float(np.percentile(vals, 75)),
    ...
    return out.sort_values(["transition_key", "arch", "bin_center_s"])
...
def main() -> None:
    ...
    print("windowed sample counts by transition, architecture, and phase:")
    print(
        all_samples.groupby(["transition_key", "arch", "phase_name"], dropna=False)
        .size()
        .rename("n_samples")
        .reset_index()
        .sort_values(["transition_key", "arch", "phase_name"])
        .to_string(index=False)
    )

    y_max = float(all_binned["power_q75"].max())
    y_min = float(all_binned["power_q25"].min())
    ...
    for row_idx, transition in enumerate(TRANSITIONS):
        ...
        if not sub.empty:
            for arch_idx, arch in enumerate(TARGET_ARCHES):
                arch_df = sub[sub["arch"] == arch].copy()
                if arch_df.empty:
                    continue
                x = arch_df["bin_center_s"].to_numpy(dtype=float)
                med = arch_df["power_median"].to_numpy(dtype=float)
                color = ARCH_COLOR[arch]

                cap_profile = (
                    arch_df.groupby("bin_center_s", dropna=False)["cap_frac"]
                    .mean()
                    .reset_index()
                    .sort_values("bin_center_s")
                )
                band_bottom = 0.01 + arch_idx * (band_height + band_gap)
    ...
    fig.suptitle("GPU Power Across Phase Transitions, Llama Scaling Runs", y=0.995, fontweight="bold", fontsize=14)
    handles, labels = axes[0].get_legend_handles_labels()
    handles.append(Line2D([0], [0], color=CAP_BG_COLOR, linewidth=1.2))
    labels.append("phase boundary")
    fig.legend(handles, labels, loc="upper center", ncol=3, frameon=False, bbox_to_anchor=(0.5, 0.965), fontsize=11)
