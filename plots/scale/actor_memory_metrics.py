"""GRPO actor-update efficiency versus memory-side metrics and SM clock."""
...
OUTPATH = Path("plots/out/scale/actor_memory_metrics.png")
...
X_SPECS = (
    ("mem_util_mean", "Training Memory Utilization (%)"),
    ("mem_clock_mhz", "Training Memory Clock (MHz)"),
    ("sm_clock_mhz", "Training SM Clock (MHz)"),
)
...
def main() -> None:
    ...
    pf_cols = [c for c in pf_cols if c in phase_fact.columns]
    ...
    pf = pf[pf["phase_name"] == "training"].copy()
    ...
    ddf["mem_clock_mhz"] = pd.to_numeric(ddf["mem_clock_mhz"], errors="coerce")
    ...
    clocks = (
        ddf.groupby(["run_id", "global_step_canonical"], dropna=False)[["mem_clock_mhz", "sm_clock_mhz"]]
        .mean()
        .reset_index()
    )

    df = sf.merge(pt, on=["run_id", "global_step_canonical"], how="inner")
    df = df.merge(pf[["run_id", "global_step_canonical", "mem_util_mean"]], on=["run_id", "global_step_canonical"], how="left")
    df = df.merge(clocks, on=["run_id", "global_step_canonical"], how="left")
    ...
    print(
        df.groupby(["family", "config", "run_id"], dropna=False)[["update_actor_ms_per_train_token", "mem_util_mean", "mem_clock_mhz", "sm_clock_mhz"]]
        .mean()
        .reset_index()
    ...
    fig, axes = plt.subplots(1, len(X_SPECS), figsize=(20.2, 5.6), sharey=True)
    for ax, (x_col, x_label) in zip(axes, X_SPECS):
        for config in CONFIG_ORDER:
            sub = df[df["config"] == config].dropna(subset=[x_col, "update_actor_ms_per_train_token"]).copy()
            if sub.empty:
                continue
            ax.scatter(
                sub[x_col],
    ...
    fig.suptitle("GRPO Actor-Update Efficiency vs Memory-Side Metrics", y=0.985, fontsize=FIGURE_TITLE_SIZE, fontweight="bold")
    ...
    manifest = build_run_manifest(
        plot_name="actor_memory_metrics",
        run_ids=run_ids,
        data_sources={
            "root": "results/monitoring_val/llama_scaling",
            "views": ["step_fact_view", "phase_timings_long", "phase_fact_view", "device_timeseries_view", "runs", "run_summary_view"],
