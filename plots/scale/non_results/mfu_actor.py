"""Actor MFU across scaling configurations, grouped by policy."""
...
OUTPATH = Path("plots/out/scale/non_results/mfu_actor.png")
...
def main() -> None:
    ...
    if "metric_perf_mfu_actor" not in wide.columns:
        raise ValueError("step_metrics_wide_curated missing metric_perf_mfu_actor")
    df = wide[wide["run_id"].astype(str).isin(run_ids)][["run_id", "global_step_canonical", "metric_perf_mfu_actor"]].copy()
    df["global_step_canonical"] = pd.to_numeric(df["global_step_canonical"], errors="coerce")
    df["metric_perf_mfu_actor"] = pd.to_numeric(df["metric_perf_mfu_actor"], errors="coerce")
    df = df.dropna(subset=["global_step_canonical", "metric_perf_mfu_actor"]).copy()
    ...
    summary = (
        df.groupby(["policy_norm", "config", "run_id"], dropna=False)["metric_perf_mfu_actor"]
        .mean()
        .reset_index(name="run_mean_mfu_actor")
        .sort_values(["policy_norm", "config", "run_id"])
    )
    print("run-level actor MFU summary:")
    ...
    y_max = float(df["metric_perf_mfu_actor"].max()) if not df.empty else 1.0
    ...
    for ax, policy in zip(axes, POLICY_ORDER):
        ...
        for config in CONFIG_ORDER:
            ...
            box_data.append(sub["metric_perf_mfu_actor"].to_numpy())
            box_pos.append(x0)
        if box_data:
            bp = ax.boxplot(
                box_data,
    ...
    axes[0].set_ylabel("perf/mfu/actor", fontsize=AXIS_LABEL_SIZE)
    ...
    fig.suptitle("Actor MFU by Hardware Configuration and Policy", y=0.985, fontsize=FIGURE_TITLE_SIZE, fontweight="bold")
    ...
    manifest = build_run_manifest(
        plot_name="mfu_actor_by_configuration",
        run_ids=run_ids,
