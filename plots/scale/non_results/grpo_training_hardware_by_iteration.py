"""Training-phase hardware diagnostics over iteration for GRPO scaling runs."""
...
OUTPATH = Path("plots/out/scale/grpo_training_hardware_by_iteration.png")
...
def main() -> None:
    ...
    by_iter = (
        df.groupby(["run_id", "config", "global_step_canonical"], dropna=False)[[metric for metric, _ in METRICS]]
        .mean()
        .reset_index()
        .sort_values(["config", "global_step_canonical"])
    )
    print("training hardware summary by run:")
    print(by_iter.groupby(["config", "run_id"], dropna=False)[[metric for metric, _ in METRICS]].mean().reset_index().to_string(index=False))
    ...
    fig.suptitle("GRPO Training Hardware Diagnostics by Iteration", y=0.985, fontsize=FIGURE_TITLE_SIZE, fontweight="bold")
    ...
    manifest = build_run_manifest(
        plot_name="grpo_training_hardware_by_iteration",
        run_ids=run_ids,
