"""GRPO update_actor time normalized by train tokens over iteration."""
...
OUTPATH = Path("plots/out/scale/grpo_update_actor_time_per_train_token_by_iteration.png")
...
def main() -> None:
    ...
    pt = pt.groupby(["run_id", "global_step_canonical"], dropna=False)["value"].sum(min_count=1).reset_index(name="update_actor_time_s")
    ...
    print("update_actor normalized summary by run:")
    print(
        df.groupby(["config", "run_id"], dropna=False)[["update_actor_time_s", "step_train_tokens_est", "update_actor_ms_per_train_token"]]
        .mean()
        .reset_index()
    ...
    for config in CONFIG_ORDER:
        sub = df[df["config"] == config].sort_values("global_step_canonical")
        if sub.empty:
            continue
        ax.plot(
    ...
    ax.set_ylabel("Update Actor Time (ms/train token)", fontsize=AXIS_LABEL_SIZE)
    ...
    fig.suptitle("GRPO Update Actor Time per Train Token by Iteration", y=0.98, fontsize=FIGURE_TITLE_SIZE, fontweight="bold")
    ...
    manifest = build_run_manifest(
        plot_name="grpo_update_actor_time_per_train_token_by_iteration",
        run_ids=run_ids,
        data_sources={
            "root": "results/monitoring_val/llama_scaling",
            "views": ["step_fact_view", "phase_timings_long", "runs", "run_summary_view"],
