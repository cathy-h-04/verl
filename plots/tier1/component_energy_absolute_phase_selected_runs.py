"""Component-level phase absolute energy for selected baseline runs.

Shows absolute GPU and CPU/DRAM energy by phase, grouped by policy inside each
platform panel.
"""
...
OUTPATH = Path("plots/out/figures/tier1/component_energy_absolute_phase_selected_runs.png")
...
def main() -> None:
    ...
    # Mean per phase-instance row keeps the chart comparable even if row counts differ slightly.
    agg = (
        phase_df.groupby(["platform", "policy_norm", "phase_bucket"], dropna=False)[["gpu_energy_j", "cpu_dram_energy_j"]]
        .mean()
        .reset_index()
        .rename(columns={"gpu_energy_j": "gpu_energy_j_mean", "cpu_dram_energy_j": "cpu_dram_energy_j_mean"})
    )

    print("absolute energy means (J) by platform/policy/phase:")
    print(agg.sort_values(["platform", "policy_norm", "phase_bucket"]).to_string(index=False))
    ...
    for ax, platform in zip(axes, PLATFORM_ORDER):
        ...
        for policy in POLICY_ORDER:
            ...
            for phase in PHASE_ORDER:
                ...
                vals_gpu.append(float(row["gpu_energy_j_mean"].iloc[0]) if not row.empty else 0.0)
                vals_cpu.append(float(row["cpu_dram_energy_j_mean"].iloc[0]) if not row.empty else 0.0)
    ...
    axes[0].set_ylabel("mean phase energy (J)")
    ...
    fig.suptitle("Phase-Level Absolute Component Energy (Selected Runs)", y=0.99)
    fig.legend(
        component_handles,
        ["GPU", "CPU/DRAM"],
