#!/bin/bash
# ================================================================
# Verl SFT Training Script - Non-validation mode (Env-driven)
# Mirrors run_verl_train_nonval.sh with SFT trainer entrypoint.
# ================================================================
...
if [ "${POLICY,,}" != "sft" ]; then
    echo "ERROR: run_verl_train_sft_nonval.sh expects POLICY=sft (got '$POLICY')"
    exit 1
fi
if [ "$NNODES" != "1" ]; then
...
if [ "$DATASET_NAME" = "rlhf-ff" ] && [ ! -f "$VAL_FILE" ]; then
    echo "ERROR: ${VAL_FILE} is required for rlhf-ff SFT runs. Please regenerate dataset split."
    exit 1
fi
...
echo "========================================"
echo "Starting SFT Training (non-validation)"
echo "========================================"
